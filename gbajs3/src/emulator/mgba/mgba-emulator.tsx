...
export type GBAEmulator = {
  ...
  getStatRoms: (name: string) => FS.Stats;
  getStatSaves: (name: string) => FS.Stats;
...
export const mGBAEmulator = (mGBA: mGBAEmulatorTypeDef): GBAEmulator => {
  ...
  return {
    ...
    getStatRoms: (name) => mGBA.FS.stat(`${paths.gamePath}/${name}`),
    getStatSaves: (name) => mGBA.FS.stat(`${paths.savePath}/${name}`),
    listSaves: () =>
      mGBA.FS.readdir(paths.savePath).filter(
        (saveName) => !fileIgnorePaths.includes(saveName)
      ),
    uploadCheats: (...args) => {
